...
const DICT: Dict = {
  ...
  // Smart Money & Copy Trading page (/smart-money)
  ...
  "compare.info.atr1h": { zh: "btc-qt 的 1 小时 ATR(14)，用于把事件振幅、止损和跨交易所偏离统一换算为小时级波动尺度。", en: "btc-qt's 1-hour ATR(14), used to normalize event amplitude, stops, and cross-venue deviation to an hourly volatility scale." },
  ...
  "compare.info.liquidationNotional": { zh: "多头与空头强平名义金额之和。金额放大表示被动去杠杆流量增加；需与 OI 变化和价格方向共同确认。", en: "Sum of long and short liquidation notional. A spike marks more forced deleveraging and should be confirmed with OI change and price direction." },
  "compare.info.liquidationCount": { zh: "OKX 历史接口返回并按日聚合的强平明细条数；它衡量事件频度，不代表所有交易所的完整爆仓笔数。", en: "Count of OKX liquidation details aggregated by day. It measures event frequency, not the complete cross-exchange liquidation count." },
  "compare.info.liqOiPercentile": { zh: "强平金额/OI 比值的滚动百分位。接近 100 表示相对未平仓规模出现极端杠杆清洗。", en: "Rolling percentile of liquidation notional divided by OI. Near 100 marks an extreme leverage flush relative to open positions." },
  "compare.info.liqDecaying": { zh: "强平流从局部峰值显著回落且继续下降时为 1，否则为 0；只表示强制流衰减，不单独构成反转信号。", en: "Equals 1 when liquidation flow has fallen materially from a local peak and keeps declining; it shows forced-flow decay, not a standalone reversal." },
  "compare.info.oiChange5mPercentile": { zh: "OI 5 分钟变化在滚动样本中的百分位。极低值代表快速去杠杆，是 btc-qt 极端事件确认条件之一。", en: "Rolling percentile of 5-minute OI change. Very low readings mark rapid deleveraging and help confirm btc-qt extreme events." },
  "compare.info.normalizedTrendScore": { zh: "24h、72h、7d 波动率调整收益按 40%/35%/25% 加权；至少两个周期同向才输出趋势分。", en: "Weighted 40%/35%/25% from 24h, 72h, and 7d volatility-adjusted returns; emitted only when at least two horizons agree." },
  "compare.info.trendAgree": { zh: "与加权趋势方向一致的周期数量，范围 0–3。数值越高，多周期趋势确认越完整。", en: "Number of horizons agreeing with the weighted trend direction, from 0 to 3. Higher values mean stronger multi-horizon confirmation." },
  "compare.info.donchianBreak": { zh: "收盘突破前 20 根 K 线上轨为 +1，跌破下轨为 -1，否则为 0；当前 K 线不参与通道计算。", en: "+1 for a close above the prior 20-bar upper channel, -1 below the lower channel, else 0; the current bar is excluded from the channel." },
  "compare.info.retZRobust": { zh: "使用滚动中位数与 MAD 计算的收益异常度，比均值/标准差 Z 分数更不容易被加密市场尖峰污染。", en: "Return anomaly normalized with a rolling median and MAD, making it less distorted by crypto-market spikes than mean/std Z-scores." },
  "compare.info.xvenueDeviation": { zh: "合约收盘价与 OKX 多交易所指数价之差，再除以 ATR60。高绝对值提示单一交易场所偏离，不等同于全市场重定价。", en: "Swap close minus the OKX multi-venue index, normalized by ATR60. Large absolute values flag venue-local dislocation rather than broad repricing." },
  "compare.info.spreadPercentile": { zh: "买卖价差在历史盘口快照中的滚动百分位。高值表示执行成本与流动性压力处于异常区间。", en: "Rolling percentile of bid/ask spread from collected book snapshots. High readings mark unusually costly execution and liquidity stress." },
  "compare.info.eventActive": { zh: "btc-qt 极端事件状态：稳健收益异常并由强平/OI 去杠杆确认后为 1，事件结束后归零。", en: "btc-qt extreme-event state: 1 after a robust return shock is confirmed by liquidation/OI deleveraging, then 0 after resolution." },
  "compare.info.eventDirection": { zh: "极端事件方向：+1 为向上冲击，-1 为向下冲击，0 表示无活动事件。", en: "Extreme-event direction: +1 upward shock, -1 downward shock, and 0 when no event is active." },
  "compare.info.eventVwap": { zh: "从事件触发开始累计的成交量加权均价，是判断价格接受或收复的成本锚。", en: "Volume-weighted price accumulated from event onset, serving as the cost anchor for acceptance or reclaim." },
  "compare.info.eventExtreme": { zh: "活动事件到当前为止的最远价格极值：下跌事件取最低价，上涨事件取最高价。", en: "Farthest price extreme reached by the active event: low for downside events and high for upside events." },
  "compare.info.reclaimFraction": { zh: "价格从事件极值向触发参考价收复的比例。接近 100% 表示冲击大部分被收回。", en: "Fraction of the move reclaimed from the event extreme toward its trigger reference. Near 100% means most of the shock was recovered." },
  "compare.info.eventVerdict": { zh: "事件结果状态：+1 接受冲击并延续，-1 拒绝冲击并收复，0 尚未判定或无事件。", en: "Event outcome state: +1 accepted/continued, -1 rejected/reclaimed, and 0 pending or inactive." },
  "compare.info.eatFearScore": { zh: "btc-qt 风格的恐惧可交易度：综合恐惧贪婪指数、下行瀑布强度与清算压力，范围 0–100。", en: "btc-qt-style tradable-fear score combining Fear & Greed, downside cascade intensity, and liquidation stress, from 0 to 100." },
  "compare.info.eatGreedScore": { zh: "btc-qt 风格的贪婪可交易度：综合恐惧贪婪指数、上行瀑布强度与清算压力，范围 0–100。", en: "btc-qt-style tradable-greed score combining Fear & Greed, upside cascade intensity, and liquidation stress, from 0 to 100." },
  ...
  "compare.group.strategySignals": { zh: "量化策略信号", en: "Quant Strategy Signals" },
  "compare.group.priceAnchors": { zh: "价格与成交锚点", en: "Price & Traded Anchors" },
  "compare.group.priceDeviation": { zh: "动量与价格偏离", en: "Momentum & Price Deviation" },
  "compare.group.trendStructure": { zh: "趋势结构", en: "Trend Structure" },
  "compare.group.trendStrength": { zh: "趋势强度与确认", en: "Trend Strength & Confirmation" },
  "compare.group.volatilityPrice": { zh: "绝对波动尺度", en: "Absolute Volatility Scale" },
  "compare.group.volatilityNormalized": { zh: "标准化波动率", en: "Normalized Volatility" },
  "compare.group.volumeActivity": { zh: "成交活跃度", en: "Trading Activity" },
  "compare.group.orderFlow": { zh: "主动成交与订单流", en: "Taker & Order Flow" },
  "compare.group.fundingRates": { zh: "资金费率与期现溢价", en: "Funding & Carry" },
  "compare.group.fundingExtremes": { zh: "资金费率极值", en: "Funding Extremes" },
  "compare.group.openInterestLevel": { zh: "未平仓合约规模", en: "Open Interest Level" },
  "compare.group.openInterestChange": { zh: "未平仓合约变化", en: "Open Interest Change" },
  "compare.group.openInterestCrowding": { zh: "OI 拥挤度", en: "OI Crowding" },
  "compare.group.traderPositioning": { zh: "交易者多空仓位", en: "Trader Positioning" },
  "compare.group.liquidationFlow": { zh: "清算流量", en: "Liquidation Flow" },
  "compare.group.liquidationStress": { zh: "清算压力与衰减", en: "Liquidation Stress & Decay" },
  "compare.group.marketLiquidity": { zh: "盘口深度", en: "Order-book Depth" },
  "compare.group.executionStress": { zh: "执行成本与盘口压力", en: "Execution & Book Stress" },
  "compare.group.eventLifecycle": { zh: "极端事件生命周期", en: "Extreme-event Lifecycle" },
  "compare.group.sentiment": { zh: "市场情绪", en: "Market Sentiment" },
  "compare.group.smartMoney": { zh: "聪明钱流向", en: "Smart-money Flow" },
  "compare.group.onchainActivity": { zh: "链上活动与流动性", en: "On-chain Activity & Liquidity" },
  "compare.group.networkSecurity": { zh: "网络安全性", en: "Network Security" },
  "compare.group.minerEconomics": { zh: "矿工经济", en: "Miner Economics" },
  "compare.group.cryptoCrossSection": { zh: "加密资产横截面", en: "Crypto Cross-section" },
  ...
  "compare.s.atr1h": { zh: "{ccy} ATR(14, 1h)", en: "{ccy} ATR(14, 1h)" },
  ...
  "compare.s.liquidationNotional": { zh: "{ccy} 强平总金额", en: "{ccy} Total Liquidation Notional" },
  "compare.s.liquidationCount": { zh: "{ccy} 强平事件数", en: "{ccy} Liquidation Event Count" },
  "compare.s.liqOiPercentile": { zh: "{ccy} 强平/OI 百分位", en: "{ccy} Liquidation/OI Percentile" },
  "compare.s.liqDecaying": { zh: "{ccy} 强平衰减状态", en: "{ccy} Liquidation Decay State" },
  "compare.s.oiChange5mPercentile": { zh: "{ccy} OI 5分钟变化百分位", en: "{ccy} OI 5m Change Percentile" },
  "compare.s.normalizedTrendScore": { zh: "{ccy} 多周期标准化趋势", en: "{ccy} Multi-horizon Normalized Trend" },
  "compare.s.trendAgree": { zh: "{ccy} 趋势同向周期数", en: "{ccy} Trend Agreement Count" },
  "compare.s.donchianBreak": { zh: "{ccy} Donchian 突破状态", en: "{ccy} Donchian Break State" },
  "compare.s.retZRobust": { zh: "{ccy} 稳健收益 Z", en: "{ccy} Robust Return Z-Score" },
  "compare.s.xvenueDeviation": { zh: "{ccy} 跨市场价格偏离", en: "{ccy} Cross-venue Deviation" },
  "compare.s.spreadPercentile": { zh: "{ccy} 价差百分位", en: "{ccy} Spread Percentile" },
  "compare.s.eventActive": { zh: "{ccy} 极端事件进行中", en: "{ccy} Extreme Event Active" },
  "compare.s.eventDirection": { zh: "{ccy} 极端事件方向", en: "{ccy} Extreme Event Direction" },
  "compare.s.eventVwap": { zh: "{ccy} 事件 VWAP", en: "{ccy} Event VWAP" },
  "compare.s.eventExtreme": { zh: "{ccy} 事件极值", en: "{ccy} Event Extreme" },
  "compare.s.reclaimFraction": { zh: "{ccy} 事件收复比例", en: "{ccy} Event Reclaim Fraction" },
  "compare.s.eventVerdict": { zh: "{ccy} 事件判定", en: "{ccy} Event Verdict" },
  "compare.s.eatFearScore": { zh: "{ccy} Eat Fear 分数", en: "{ccy} Eat Fear Score" },
  "compare.s.eatGreedScore": { zh: "{ccy} Eat Greed 分数", en: "{ccy} Eat Greed Score" },
